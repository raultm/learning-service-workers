...
const CACHE_NAME = 'lsw005-offline-cache';
...
self.addEventListener('install', function (event) {
    event.waitUntil(
        ...
        .then(async cache => {
            ...
            const leadingSlashRemoved = raw.offlinefirst.map(url => url);
            console.log(leadingSlashRemoved)
            do {
...
self.addEventListener('fetch', function (event) {
    ...
    //console.log(this.registration.scope)
    //console.log(event.request);
    const relativeUrl = event.request.url.replace(this.registration.scope, '')
    //console.log(relativeUrl)
    console.table({ url: relativeUrl, "Offline First?": raw.offlinefirst.includes(relativeUrl), "Online First?" : raw.onlinefirst.includes(relativeUrl) })
